...
void MadgwicksFilter :: magnRead(int16_t magnetometerDataRaw[], float magnetometerData[]){
  ...
  magnetometerData[0] = (((float) magnetometerDataRaw[0]) - (-92)) * 1.0;
  magnetometerData[1] = (((float) magnetometerDataRaw[1]) - (-48)) * 1.02702702703;
  magnetometerData[2] = (((float) magnetometerDataRaw[2]) - (-442)) * 0.974358974359;
...
/* =====================================================================================
                            DATA READ FUNCTIONS NON-BLOCKING
====================================================================================== */


uint8_t MadgwicksFilter :: accelRead_NB(int16_t accelerometerDataRaw[], float accelerometerData[]){
    ...
    if (acc_sent == 0){
        ...
        Wire.write(ACCEL_REGISTER_OUT_X_L_A | 0x80);   // this 'OR' activates the continous read function
        Wire.endTransmission();
        Wire.sendRequest(ACCEL_ADDR, 6, I2C_NOSTOP);
        acc_sent = 1;
    }

    // Return with STILL_WAITING code if the bus is busy or not all the data has arrived.
    if (Wire.available() < 6 || Wire.done() != 1) return 1;
    ...
    // Reset the I2C sent flag
    acc_sent = 0;

    // return with DONE code
    return 0;
}



uint8_t MadgwicksFilter :: magnRead_NB(int16_t magnetometerDataRaw[], float magnetometerData[]){
    ...
    if (magn_sent == 0){
        ...
        Wire.write(MAGN_REGISTER_OUT_X_H_M);
        Wire.endTransmission();
        Wire.sendRequest(MAGN_ADDR, 6, I2C_NOSTOP);
        magn_sent = 1;
    }
...
uint8_t MadgwicksFilter :: gyroRead_NB(int16_t gyroscopeDataRaw[], float gyroscopeData[]){
    ...
    if (gyro_sent == 0){
        ...
        Wire.write(GYRO_REGISTER_OUT_X_L | 0x80);  // The 'OR' sets the auto-increment bit
        Wire.endTransmission();
        Wire.sendRequest(GYRO_ADDR, 6, I2C_NOSTOP);
        gyro_sent = 1;
    }
...
uint8_t MadgwicksFilter :: getStatus(){
...
void  MadgwicksFilter :: begin(void){
    ...
    //Initialze the quaternion with an educated Guess
...
void MadgwicksFilter :: step(void){
    ...
    // This is meant to be an state machine so that the data gathering and filter process
    // can run without blocking the main thread.
    // The step function has to be called at least 4 times before the angles are updated (inside the filter function).


    // if 'status' flag is 0, last filtering done, Start the the reading procedure with the accelerometer
    if (status == 0){
        if (accelRead_NB(_accelDataRaw, _accelData) == 0) status = 1;   // if the read function returns 0, data has been updated, move to the next state.
        else return;                                                    // else return to main while we wait for the sensor.
    }

    // if 'status' flag is 1, accelerometer data is up to date, proceed to read the gyroscope.
    if (status == 1){
        if (gyroRead_NB(_gyroDataRaw, _gyroData) == 0) status = 2;      // if the read function returns 0, data has been updated, move to the next state.
        else return;                                                    // else return to main while we wait for the sensor.
    }
    // if 'status' flag is 2, gyroscope data is up to date, proceed to read the magnetometer.
    if (status == 2){
        if (magnRead_NB(_magnDataRaw, _magnData) == 0) status = 3;      // if the read function returns 0, data has been updated, move to the next state.
        else return;                                                    // else return to main while we wait for the sensor.
    }

    // if 'status' flag is 3, all sensor data is up to date, proceed to iterate the filter.
    if (status == 3){
        // Actualize time variables
        dt = dt_timer/1000000.0;
