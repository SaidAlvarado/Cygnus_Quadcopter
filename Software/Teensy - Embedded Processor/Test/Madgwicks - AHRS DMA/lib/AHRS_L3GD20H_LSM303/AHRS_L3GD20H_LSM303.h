...
class MadgwicksFilter {
  ...
  private:
    ...
    //Time keeping
    float dt;
    elapsedMicros dt_timer;
    // Non-blocking state machine status
    uint8_t status = 0;
    /*
    status 0  --> first state, no data acquired.
    status 1  --> Accelerometer data updated. go and request the gyroscope data.
    status 2  --> Gyroscope data updated. go and request the magnetometer data.
    status 3  --> Magnetometer data updated. go ahead and run the filter.
    */
    // Flag for the Non-blocking functions so that you can call the function a million times, but it will send the I2C request just once.
    uint8_t acc_sent = 0;   // 0 -> I2C request to be sent. 1 -> request sent, awaiting response
    uint8_t gyro_sent = 0;  // 0 -> I2C request to be sent. 1 -> request sent, awaiting response
    uint8_t magn_sent = 0;  // 0 -> I2C request to be sent. 1 -> request sent, awaiting response
    ...
    //Sensor Data read, blocking version
    ...
    //Sensor Data read, NON-blocking version
    uint8_t accelRead_NB(int16_t accelerometerDataRaw[], float accelerometerData[]);    // 0 -> Ready, 1 -> Waiting for data
    uint8_t gyroRead_NB(int16_t gyroscopeDataRaw[], float gyroscopeData[]);             // 0 -> Ready, 1 -> Waiting for data
    uint8_t magnRead_NB(int16_t magnetometerDataRaw[], float magnetometerData[]);       // 0 -> Ready, 1 -> Waiting for data
